...
What can we do in cases like example \ref{fig:excelmess}? We can enter all formulas manually. Or, which is highly recommended, first tidy the data, before we start working with it. 
...
2. We accidentally leave out a cell in a range in Figure \@ref(fig:19). Excel recognizes that the range *B2:B3* included in the formula in  cell *E2*  should probably also have included the cell *B1* and gives us a warning. Again, we can right-click on the cell to read the warning. 
...
We create a chart in Excel by selecting "Insert" in the upper menu as shown in Figure \@ref(fig:e1). 
...
#### Combing chart types{-}
In some situations we would like to combine chart types. Again, there are several ways to to do this. One approach is as shown in Figure \@ref(fig:29), where we right-click on an existing series and select "Change series chart type".
...
We can add elements such as axes titles, a legend and a chart title. This can be done through the "Add Chart Element" menu, as shown in Figure \ref{fig:change}
